...
const { deleteUser } = require('./utils/deleteFunction');
...
describe('Post /api/v1/user/signUp', () => {
  test('Should register a user and return a json response', async () => {
    const { statusCode, body } = await request(app).post('/api/v1/user/signUp').send({
      firstName: 'Dump',
      lastName: 'Dump',
      email: 'Dump3@gmail.com',
      userPassword: 'dump@123',
    });
    ...
    expect(body.message).toMatch(/Success/i);
    expect(statusCode).toBe(200);
    deleteUser(body.user.email);
  });

  test('Should respond with 400 code and not exist message', async () => {
    const { statusCode, body } = await request(app).post('/api/v1/user/signUp').send({
      firstName: 'Dump',
      lastName: 'Dump',
      email: 'hamzababar3@gmail.com',
